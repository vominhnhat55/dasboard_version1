...
def render(filters: dict):
    ...
    with c1: kpi_card("📦 Danh mục #1", top_cat["category"],
                      f"DS: {fmt_vnd(top_cat['revenue'])} ({top_cat['share_pct']:.1f}%)", "pos")
    with c2: kpi_card("📊 Số danh mục",       str(len(cat)))
    with c3: kpi_card("🔖 SKU TB / danh mục", f"{cat['skus'].mean():.0f}")
    ...
    with col1:
        ...
        chart_with_data(
            ...
            display_cols={"category":"Danh mục","revenue":"Doanh số (VND)","share_pct":"Tỷ trọng (%)"},
    ...
    chart_with_data(
        ...
        display_cols={"category":"Danh mục","week":"Tuần","revenue":"Doanh số (VND)","qty":"SL"},
    ...
    if len(weeks_avail) >= 2:
        ...
        cat_piv["growth"] = (cat_piv[w_last] - cat_piv[w_first]) / (cat_piv[w_first] + 1) * 100
        ...
        with col1:
            ...
            if bad.empty: alert_box("✅ Tất cả danh mục đều tăng trưởng dương", "g")
            for _, r in bad.iterrows():
                alert_box(f"📉 <b>{r['category']}</b> — giảm {abs(r['growth']):.1f}% "
                          f"(tuần {w_first}→{w_last})", "r")
        with col2:
            st.markdown("**🚀 Danh mục tăng trưởng tốt (>30%)**")
            good = cat_piv[cat_piv["growth"] >= 30].sort_values("growth", ascending=False)
            if good.empty: alert_box("ℹ️ Không có danh mục tăng mạnh >30%", "w")
    ...
    fig4.update_traces(textposition="top center", textfont=dict(color="#fff", size=10))
    chart_with_data(
